...
export default function App() {
  ...
  const handleExportJSON = async () => {
    try {
      ...
      const serializedBlobs = await Promise.all(allBlobs.map(async b => {
      ...
      notify("Failed to export data. Check console for details.", "error");
    }
  };
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (!data.nodes || !Array.isArray(data.nodes)) {
          throw new Error("Invalid backup file format: 'nodes' array is missing or invalid.");
        }

        // Clean and validate nodes
        const cleanedNodes = data.nodes.map((n: any) => {
          if (!n.id || !n.type) throw new Error(`Node ${n.name || 'unknown'} is missing id or type.`);
          
          // Normalize type to enum values
          let nodeType = NodeType.FILE;
          const rawType = String(n.type).toUpperCase();
          if (rawType === 'FOLDER') nodeType = NodeType.FOLDER;
          else if (rawType === 'FILE') nodeType = NodeType.FILE;
        ...
        const uniqueNodes = Array.from(new Map(cleanedNodes.map(n => [n.id, n])).values());

        setConfirmation({
          title: "Import Data",
          message: `Found ${uniqueNodes.length} items in backup. This will PERMANENTLY REPLACE your current data. Continue?`,
          onConfirm: async () => {
            try {
              await db.transaction('rw', db.nodes, db.blobs, async () => {
                await db.nodes.clear();
                await db.blobs.clear();
                await db.nodes.bulkPut(uniqueNodes as FileNode[]);
              ...
              console.error("Import failed during transaction:", err);
